...
// Filtrar quando o select mudar
if (serviceFilter)
  serviceFilter.addEventListener("change", applyFilterAndRender);
...
// Aplica filtro selecionado e renderiza
function applyFilterAndRender() {
  const sel = serviceFilter ? serviceFilter.value : "all";
  if (!allData) return renderTable([]);
  let filtered = allData;
  if (sel && sel !== "all") {
    const match = sel.toLowerCase();
    filtered = allData.filter((c) => (c.service || "").toLowerCase() === match);
  }
...
const renderTable = (data) => {
  ...
  data.forEach((client) => {
    ...
    row.innerHTML = `
            ...
            <td class="px-4 py-4 text-center"><button onclick="handleEdit(${
              client.id
            })" class="text-blue-600 hover:bg-blue-50 p-2 rounded"><i class="fas fa-edit"></i></button></td>
            <td class="px-4 py-4 text-center"><button onclick="handleRemove(${
              client.id
            }, '${
      client.name
    }')" class="text-red-500 hover:bg-red-50 p-2 rounded"><i class="fas fa-trash-alt"></i></button></td>
        `;
